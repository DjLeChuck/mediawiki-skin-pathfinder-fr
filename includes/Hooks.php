    ...
    SkinPageReadyConfigHook
{
    ...
    private static function getActiveABTest($config)
    {
        ...
        if (!array_key_exists('unsampled', $ab['buckets'])) {
            throw new RuntimeException(
                'Invalid PathfinderFRWebABTestEnrollment value: Must define an `unsampled` bucket.'
            );
        } else {
            // check bucket values.
            foreach ($ab['buckets'] as $bucketName => $bucketDefinition) {
                if (!is_array($bucketDefinition)) {
                    throw new RuntimeException(
                        'Invalid PathfinderFRWebABTestEnrollment value: Buckets should be arrays'
                    );
                }
     ...
     * @internal used inside Hooks::onSkinTemplateNavigation
     */
    private static function updateViewsMenuIcons(&$content_navigation)
    {
    ...
    public static function onSkinTemplateNavigation($sk, &$content_navigation)
    {
        ...
        self::updateViewsMenuIcons($content_navigation);
    ...
    public function onResourceLoaderSiteStylesModulePages($skin, &$pages): void
    {
        ...
        if ($config->get('PathfinderFRShareUserScripts')) {
            $pages['MediaWiki:PathfinderFR.css'] = ['type' => 'style'];
        }
    }
    ...
    public function onResourceLoaderSiteModulePages($skin, &$pages): void
    {
        $config = MediaWikiServices::getInstance()->getMainConfig();
        if ($config->get('PathfinderFRShareUserScripts')) {
            $pages['MediaWiki:PathfinderFR.js'] = ['type' => 'script'];
        }
    }
